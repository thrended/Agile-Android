...
////        LazyColumn(
////            content = {
////            items(state.availSprints.size) {
////                state.availSprints.entries.forEach { s ->
////                    ListItem(
////                        modifier = Modifier
////                            .clickable { viewModel.onEvent(TasksEvent.ChangeSprint(s.key)) },
////                        text = { Text("Sprint ${s.key} - ${s.value}")}
////                    )
////                }
////            } }
////        )
...
//            when(event) {
//                is TasksViewModel.UIEvent.ToastMsg -> {
...
//                        LazyColumn(
//                            content = {
//                                itemsIndexed(state.sprintList) { i, sp ->
//                                    val k = sp.sprintId ?: 1L
//                                    ListItem(
//                                        modifier = Modifier
//                                            .clickable {
//                                                scope.launch {
...
//                    title3 = if(state.sprintStarted) "${state.timeRem} Days Remaining" else "Duration of sprint: ${state.timeRem} Days"
//                )
//
//            },
//            drawerContent = {
...
//                    var textSize by remember { mutableStateOf(ID.Zero)}
...
//                            readOnly = !canEdit,
//                            value = if(state.currentSprint?.info?.cloned == true) "${state.sprintTitle} [CLONE]" else state.sprintTitle,
//                            leadingIcon = {
//                                Icon(
...
//                if(state.allTasks.isNotEmpty() && state.sprintList.isNotEmpty()) {
...
//                                        Toast.makeText(
//                                            context,
//                                            "Navigating to EditTaskScreen + ?sid=${item.task.sprintId}&id=${item.task.taskId}&taskColor=${item.task.color}",
//                                            Toast.LENGTH_LONG
//                                        ).show()
//                                        navController.navigate(
//                                            Screen.AddEditTaskScreen.route +
//                                                    "?sid=${item.task.sprintId}&id=${item.task.taskId}&taskColor=${item.task.color}"
//                                        )
//                                    },
